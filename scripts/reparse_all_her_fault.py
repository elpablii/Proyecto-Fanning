...
pdf_files = glob.glob('Vocabularios/2026/Palabras desconocidas de All Her Fault*.pdf')
episodes_data = []
...
for pdf in sorted(pdf_files):
    ...
    for line in lines:
        ...
        # Match 'Episode 1 (54 words/phrases):' or 'Episode 1: Title (54 words/phrases)'
        ep_match = re.match(r'^Episode (\d+)(?:[:\s]*(.*?))?\((\d+)\s*words?', line, re.IGNORECASE)
        if ep_match:
            ep_num = ep_match.group(1)
            count = int(ep_match.group(3))
            current_ep = {
                "name": f"All Her Fault (Episode {ep_num})",
                "count": count,
                "vocabulary": []
            }
...
out_path = 'fanning-dashboard/public/data/pelis/All Her Fault.json'
